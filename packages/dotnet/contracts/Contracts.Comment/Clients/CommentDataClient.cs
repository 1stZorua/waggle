...
namespace Waggle.Contracts.Comment.Clients
{
    public class CommentDataClient : ICommentDataClient
    {
        ...
        public async Task<Result<GetCommentCountsResponse>> GetCommentCountsAsync(GetCommentCountsRequest request)
        {
            try
            {
                var response = await _client.GetCommentCountsAsync(request);
                return Result<GetCommentCountsResponse>.Ok(response);
            }
            catch (RpcException ex)
            {
                return GrpcExceptionHelper.HandleRpcException<GetCommentCountsResponse>(ex);
            }
        }

        public async Task<Result<GetReplyCountsResponse>> GetReplyCountsAsync(GetReplyCountsRequest request)
        {
            try
            {
                var response = await _client.GetReplyCountsAsync(request);
                return Result<GetReplyCountsResponse>.Ok(response);
            }
            catch (RpcException ex)
            {
                return GrpcExceptionHelper.HandleRpcException<GetReplyCountsResponse>(ex);
            }
        }
