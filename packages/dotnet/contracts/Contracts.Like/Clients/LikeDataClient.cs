...
namespace Waggle.Contracts.Like.Clients
{
    public class LikeDataClient : ILikeDataClient
    {
        ...
        public async Task<Result<GetLikeCountsResponse>> GetLikeCountsAsync(GetLikeCountsRequest request)
        {
            try
            {
                var response = await _client.GetLikeCountsAsync(request);
                return Result<GetLikeCountsResponse>.Ok(response);
            }
            catch (RpcException ex)
            {
                return GrpcExceptionHelper.HandleRpcException<GetLikeCountsResponse>(ex);
            }
        }
