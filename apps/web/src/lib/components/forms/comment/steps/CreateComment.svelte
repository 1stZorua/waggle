<script lang="ts">
	...
	const replyingTo = comment?.user?.username ?? null;

	const { form, errors, enhance } = superForm(data.createCommentForm, {
		onResult: ({ result }) => {
...
>
	...
	<div class="gap-xl flex items-center justify-between">
		<div class="flex flex-col">
			<TextLarge>{replyingTo ? `Reply to @${replyingTo}` : 'Add a comment'}</TextLarge>
		</div>
