<script lang="ts">
	...
	import { Dropdown, Icon, Separator, Carousel } from '$components/shared/other';
	...
	const imageUrls = $derived(
		$form.images ? $form.images.map((img) => URL.createObjectURL(img)) : []
	);
	...
	<div class="gap-md flex min-h-0 flex-1 flex-col">
		<div class="gap-md flex min-h-0 flex-1">
			{#if imageUrls.length > 0}
				<div class="h-full min-h-0 flex-[2.5]">
					<Carousel
						images={imageUrls}
						className="h-full"
